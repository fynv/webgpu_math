...
{
    let shaderModule = engine_ctx.device.createShaderModule({ code: get_shader_radix_scan() });
    let bindGroupLayouts = [engine_ctx.cache.bindGroupLayouts.radixScan];
...
export async function test()
{
    ...
    let buf_indices1 = engine_ctx.createBuffer0(count * 4, GPUBufferUsage.STORAGE);
    let buf_indices2 = engine_ctx.createBuffer0(count * 4, GPUBufferUsage.STORAGE);
    let buf_workgroup_counter = engine_ctx.createBuffer0(4, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST);
    let buf_workgroup_state = engine_ctx.createBuffer0(num_groups_radix_scan * 5 *4, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST);
    ...
    let bindGroupLayoutRadixScan = engine_ctx.device.createBindGroupLayout({ entries: layout_entries_radix_scan });
    engine_ctx.cache.bindGroupLayouts.radixScan = bindGroupLayoutRadixScan;    

    let pipeline_radix_scan = GetPipelineRadixScan();
    ...
    let bind_group_radix_scan = new Array(2);
    ...
    for (let i=0; i<2; i++)
    {
        ...
        bind_group_radix_scan[i] = engine_ctx.device.createBindGroup({ layout: bindGroupLayoutRadixScan, entries: group_entries_radix_scan});
    ...
    {
        {
            ...
            const group_state = new Int32Array(num_groups_radix_scan*5, 0);
        ...
        {
            let num_groups = (count + WORKGROUP_SIZE -1)/WORKGROUP_SIZE;
            const passEncoder = commandEncoder.beginComputePass();
